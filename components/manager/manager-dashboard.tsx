...
export function ManagerDashboard({ userEmail }: ManagerDashboardProps) {
  ...
  // Derived state
  const allRequests = (listIds || []).map((id) => requestsMap[id]).filter(Boolean) as SupportRequest[]
  ...
  // Calculate stats based on ALL fetched requests, not just the top 5
  const stats = {
    total: allRequests.length,
    new: allRequests.filter((r) => r.status === "new").length,
    verified: allRequests.filter((r) => r.verification_status === "verified").length,
    highPriority: allRequests.filter((r) => r.priority === "high").length,
  }

  return (
    <div className="min-h-screen bg-background">
      <ManagerHeader userEmail={userEmail} />
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        ...
        <Card>
          <CardHeader className="px-6 py-4 border-b border-border">
            <CardTitle className="text-base font-medium">Recent Requests (Top 5)</CardTitle>
          </CardHeader>
