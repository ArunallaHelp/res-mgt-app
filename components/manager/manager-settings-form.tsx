...
import { updateManagerProfile } from "@/app/actions/update-manager"
import { Loader2 } from "lucide-react"
import { toast } from "sonner"

interface ManagerSettingsFormProps {
  initialData: ManagerApplication
}

export function ManagerSettingsForm({ initialData }: ManagerSettingsFormProps) {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)

  // State for conditional "Other" fields
  const [jobRole, setJobRole] = useState<string>(initialData.job_role || "")
  const [highestQualification, setHighestQualification] = useState<string>(initialData.highest_qualification || "")
  
  // Multi-select states
  const [professionalSkills, setProfessionalSkills] = useState<string[]>(initialData.professional_skills || [])
  const [supportTypes, setSupportTypes] = useState<string[]>(initialData.support_types || [])
  const [gradeLevels, setGradeLevels] = useState<string[]>(initialData.grade_levels || [])
  const [availableDays, setAvailableDays] = useState<string[]>(initialData.available_days || [])
  const [availableTimeSlots, setAvailableTimeSlots] = useState<string[]>(initialData.available_time_slots || [])
  const [supportMethods, setSupportMethods] = useState<string[]>(initialData.support_methods || [])
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl mx-auto py-8">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Input id="email" name="email" type="email" disabled value={initialData.email} className="bg-muted" />
              <p className="text-xs text-muted-foreground">Email cannot be changed</p>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="phone">Mobile / WhatsApp Number</Label>
              <Input id="phone" name="phone" required defaultValue={initialData.phone} disabled className="bg-muted" />
              <p className="text-xs text-muted-foreground">Number cannot be changed</p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="grid gap-2">
              ...
              <Input id="nearest_town" name="nearest_town" required defaultValue={initialData.nearest_town} />
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Select name="job_role" required value={jobRole} onValueChange={setJobRole}>
                <SelectTrigger>
              ...
              {jobRole === "Other" && (
                <Input name="other_role" placeholder="Specify your role" className="mt-2" required defaultValue={initialData.other_role} />
              )}
            </div>

            <div className="grid gap-2">
              <Label htmlFor="experience_years">Years of Experience *</Label>
              <Select name="experience_years" required defaultValue={initialData.experience_years}>
                <SelectTrigger>
          ...
          <div className="grid gap-2">
            ...
            <Select name="highest_qualification" required value={highestQualification} onValueChange={setHighestQualification}>
              <SelectTrigger>
            ...
            {highestQualification === "Other" && (
              <Input name="other_qualification" placeholder="Specify qualification" className="mt-2" required defaultValue={initialData.other_qualification} />
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            {professionalSkills.includes("Other") && (
              <Input name="other_skill" placeholder="Specify other skills" className="mt-2" defaultValue={initialData.other_skill} />
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input id="subjects" name="subjects" required defaultValue={initialData.subjects} placeholder="e.g. Maths, Science, English, Commerce, ICT (Separate by commas)" />
          </div>

          <div className="space-y-2">
            <Label htmlFor="other_subject">Other Subject (Optional)</Label>
            <Input id="other_subject" name="other_subject" defaultValue={initialData.other_subject} placeholder="Any other subjects..." />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <RadioGroup name="teaching_mode" required defaultValue={initialData.teaching_mode} className="flex flex-col space-y-1">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea id="volunteering_experience" name="volunteering_experience" defaultValue={initialData.volunteering_experience} placeholder="Describe any past experience..." />
          </div>

          <div className="space-y-2">
            <Label htmlFor="preferences_limitations">Any preferences or limitations</Label>
            <Textarea id="preferences_limitations" name="preferences_limitations" defaultValue={initialData.preferences_limitations} placeholder="e.g. Can only teach in Sinhala medium..." />
          </div>

          <div className="space-y-2">
            <Label htmlFor="comments">Comments / Special Notes</Label>
            <Textarea id="comments" name="comments" defaultValue={initialData.comments} placeholder="Any other details..." />
          </div>
        </CardContent>
      </Card>
